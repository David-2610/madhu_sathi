...
class BuyerRepository(
    ...
    suspend fun addToCart(productId: String, quantity: Int = 1): ApiResult<CartDto> = withContext(Dispatchers.IO) {
        try {
            val intId = productId.filter { it.isDigit() }.toIntOrNull() ?: 1
            val cart = apiService.addToCart(AddToCartRequest(intId))
            ApiResult.Success(cart)
        } catch (e: Exception) {
            NetworkErrorMapper.map(e)
        }
    ...
    suspend fun checkout(): ApiResult<OrderDto> = withContext(Dispatchers.IO) {
        try {
            val order = apiService.checkout(CheckoutRequest())
            dao.insertOrders(
                listOf(
                    OrderEntity(
                        id = order.id,
    ...
    suspend fun confirmPayment(orderId: String, mockSuccess: Boolean = true): ApiResult<OrderDto> = withContext(Dispatchers.IO) {
        try {
            val order = apiService.confirmPayment(orderId, PaymentConfirmRequest(mockSuccess = mockSuccess))
            ApiResult.Success(order)
        } catch (e: Exception) {
            NetworkErrorMapper.map(e)
        }
