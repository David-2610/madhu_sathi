...
async def get_telemetry_history(
    ...
    if skip == 0:
        try:
            from app.services.iot_bridge import sync_hive_from_iot
            await sync_hive_from_iot(hive_id)
        except Exception as exc:
            logger.debug("IoT sync on telemetry fetch skipped: %s", exc)
...
async def get_hive_health(
    ...
    try:
        from app.services.iot_bridge import sync_hive_from_iot
        await sync_hive_from_iot(hive.id)
    except Exception as exc:
        logger.debug("IoT sync on health check skipped: %s", exc)
